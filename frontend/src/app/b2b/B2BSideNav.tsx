import { FaBell, FaBuilding, FaBullhorn, FaHeadset, FaListAlt, FaRupeeSign, FaShoppingCart, FaSignOutAlt, FaTachometerAlt, FaUsers, FaWallet } from "react-icons/fa";
...
export default function B2BSideNav() {
  ...
  // Get the current path after /b2b/
  const current = location.pathname.replace(/^\/b2b\/?/, "");

  return (
    <aside className="w-64 bg-white shadow-lg hidden md:flex flex-col h-screen fixed top-0 left-0 z-30">
      <div className="h-16 flex items-center justify-center font-bold text-xl" style={{ color: B2B_PRIMARY }}>
        B2B Panel
      </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center px-4 py-2 rounded transition font-medium ${
                isActive
                  ? "text-white"
                  : "hover:bg-[#7a1335]/10 text-gray-700"
              }`}
              style={isActive ? { background: "#7a1335", fontWeight: 700 } : {}}
            >
              {item.icon}
              {item.label}
            </Link>
      ...
      <div className="px-4 py-4 mt-auto">
        <Link
          to={logoutItem.to}
          className="flex items-center px-4 py-2 rounded transition font-medium text-red-600 hover:bg-red-50"
        >
          {logoutItem.icon}
          {logoutItem.label}
        </Link>
